...
async function runRequests() {
    ...
    const createStudentWithRandomCampusAndSubject = async (name: string, lastname: string, header = axiosHeaderAdminAuth) => {
        let userId;
        let studentId;

        await axios.get(HOST + "/university/user", header).then((response) => {
            userId = response?.data?.d?.[0]?.id
        })

        console.log(`userId:${JSON.stringify(userId, null, 2)}`);
        await axios.post(HOST + "/university/student", { name: name, lastname: lastname, user: userId }, header).then((response) => {
        ...
        await axios.post(HOST + "/university/student__studies__subject", { student: studentId, studies__subject: campus['subjectId'] }, header).catch((err: any) => {
            console.log(`Should not error ${err}`)
        })

        await axios.post(HOST + "/university/student__is_member_of__campus", { student: studentId, is_member_of__campus: campus['subjectId'] }, header).catch((err: any) => {
            console.log(`Should not error ${err}`)
        })
    }
    ...
    createStudentWithRandomCampusAndSubject('Grace', 'Hopper', axiosHeaderStudentAuth);
    // createStudentWithRandomCampusAndSubject('Grace', 'Jones');

    // students can get top 5 students
    await axios.get(HOST + "/university/student", axiosHeaderStudentAuth).then((response) => {
    ...
    // await axios.get(HOST + "/university/user", axu).catch((err) => {
